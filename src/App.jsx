...
import { Search, MapPin, Star, Plus, X, Briefcase, Building2, ArrowRight, Sparkles, LogOut, Loader2, Image, Pencil, Trash2, Flag, Camera, User, Heart, Share2, ShieldCheck, Navigation, Eye, Bell, ClipboardList } from "lucide-react";
      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          {listing.portfolio_link && (
            ...
            >
              <Image size={12} /> {isJob ? "Apply link" : "Portfolio"}
            </a>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-6 z-50" onClick={() => setShowAbout(false)}>
            <GlassCard className="w-full max-w-lg p-7 bg-[#0D1310]/95 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-5 text-sm text-white/70 leading-relaxed">
                <p>
                  In Lagos, the best hustlers and businesses are usually found through a WhatsApp status, a friend's recommendation, or pure luck. If you're not in the right group chat, you miss out, even if you're exactly what someone's looking for.
                </p>
                ...
                <div>
                  ...
                  <p>
                    Places like Esteehi Hotel & Lounge already trust us. Listing here means Lagos youth searching "hotels in Akute" or "food near me" find you first, not a competitor with a bigger ad budget. It's a shopfront that works while you sleep.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1.5 list-none">
                    ...
                    <li className="flex items-start gap-2"><span className="text-emerald-300 mt-0.5">•</span> Direct WhatsApp contact, no app-locked chat to babysit</li>
                    ...
                    <li className="flex items-start gap-2"><span className="text-emerald-300 mt-0.5">•</span> Built for Lagos specifically — areas, prices, and people you actually recognize</li>
                  </ul>
                </div>

                <p className="text-white/50 text-xs pt-2 border-t border-white/[0.07]">
                  No middleman. No wahala. Just Lagos hustlers and businesses, in one place.
                </p>
              </div>
            </GlassCard>
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-6 z-50" onClick={() => { setShowPostModal(false); setEditingId(null); setCoverFiles([]); }}>
          <GlassCard className="w-full max-w-md p-6 bg-[#0D1310]/95 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handlePost} className="space-y-3">
              ...
              {(mode === "gig" || mode === "job") && (
                <input
                  placeholder={mode === "job" ? "Apply link or email (optional)" : "Portfolio link (Instagram, Drive, Behance...)"}
